...
import type { Question, Block } from "@/lib/types";
import { questionStatusApi, mistakesApi } from "@/lib/api";

interface QuestionGroupProps {
  /** Block info (if block-based grouping) */
  block?: Block;
  /** All questions in this block/group */
  questions: Question[];
  /** Simple group: start/end numbers */
  groupIndex?: number;
  startNumber?: number;
  endNumber?: number;
  defaultOpen?: boolean;
...
export default function QuestionGroup({
  ...
  // Title from block or fallback
  const title = block?.title
    ? block.title
    : (startNumber && endNumber ? `第${startNumber}-${endNumber}题` : `题目组 ${(groupIndex ?? 0) + 1}`);
  const subtitle = block?.question_type || "";
    ...
    <div className="bg-card rounded-2xl shadow-sm border border-border overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {subtitle && (
            <span className="text-xs text-muted-foreground bg-muted/50 rounded px-1.5 py-0.5">{subtitle}</span>
          )}
          <span className="text-xs text-muted-foreground">({questions.length} 道)</span>
        </div>
        {open ? (
      ...
      {open && (
        <div className="divide-y divide-border border-t border-border">
          {questions.map((q) => {
            const answer = q.child_answer || q.student_answer;
            return (
                      ...
                      if (newSet.has(q.question_id)) {
                        ...
                        try {
                          ...
                          if (resp.ok && resp.data) {
                            const record = resp.data.find((m: any) => m.question_id === q.question_id);
...
export function groupByBlocks(questions: Question[], blocks: Block[]): { block: Block; questions: Question[] }[] {
  if (!blocks || blocks.length === 0) return [];
  const sorted = [...blocks].sort((a, b) => a.order - b.order);
  return sorted.map((b) => {
    const qs = questions.filter((q) => q.block_id === b.block_id);
    return { block: b, questions: qs };
  }).filter((g) => g.questions.length > 0);
}

/** Group questions by numeric range (fallback) */
